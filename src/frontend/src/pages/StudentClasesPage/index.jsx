...
const StudentClasesList = () => {
  ...
  useEffect(() => {
    const fetchClases = async () => {
      try {
        const data = await studentService.getClases();
        setClases(data.data);
      } catch (error) {
  ...
  return (
    <div className="mt-6">
      <ClasesGrid clases={clases} onImportSuccess={() => {}} isStudent={true} />
    </div>
  );
};

const StudentClaseDetails = ({ idClase }) => {
  ...
  return (
    <div className="w-full">
      <section className="container mx-auto max-w-5xl px-4 py-4 md:py-6 space-y-6">
        <StudentTabs />
        {idClase ? <StudentClaseDetails idClase={idClase} /> : <StudentClasesList />}
      </section>
    </div>
  );
};
